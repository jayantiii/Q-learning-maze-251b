...
class MazeEnvironment:    
    def __init__(self, maze, init_position, goal, vision_range=2, masked=False):  # Masked Q-Learning
        ...
        # Masked Q-Learning: Define the vision range and toggle masking
    ...
    def state_update(self, action):
        ...
        # if the move goes out of the maze or to a wall, the
        # reward is -1
    ...
    def state(self):  
        if self.masked:  # Masked Q-Learning: Return only a limited view of the maze
    ...
    def get_masked_state(self):  
        ...
        masked_view = np.full(self.maze.shape, -1)  # Default to -1 (unseen areas)
        masked_view[x_min:x_max, y_min:y_max] = self.maze[x_min:x_max, y_min:y_max]
        masked_view[tuple(self.current_position)] = 2  # Mark agent position
